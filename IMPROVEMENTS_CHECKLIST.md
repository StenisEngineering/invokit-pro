...
- [~] **7. Richer empty states** — invoice/receipt list DONE 2026-07-18: context-aware ("No invoices yet — create your first…" vs "No matching invoices — try a different filter"). Clients/Catalogue/Recurring/Analytics still to do.
- [~] **Communicative UI (new, user-requested)** — app should explain itself. DONE 2026-07-18: Dashboard KPIs now describe insights (Total Paid "received from N paid", Outstanding flags "N overdue · £X", raw count → **Collection Rate %**, Avg "across N invoices") + explanatory `title` tooltips per card. NEXT: section helper one-liners (Catalogue/Recurring/etc.), tooltips on key controls (incl. mobile tap-to-reveal since `title` is hover-only), inline hints in the editor.
